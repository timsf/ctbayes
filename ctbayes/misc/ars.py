...
def sample(f: Callable[[Union[float, np.ndarray]], Union[float, np.ndarray]],
           df: Callable[[Union[float, np.ndarray]], Union[float, np.ndarray]],
           lb: float = -np.inf,
           ub: float = np.inf,
           init: np.ndarray = None,
           max_points: int = 100,
           ome: np.random.Generator = np.random.default_rng(),
           max_draws: int = np.inf) -> Iterator[float]:
...
def brace(f: Callable[[np.ndarray], np.ndarray], lb: float = -np.inf, ub: float = np.inf) -> np.ndarray:
    ...
    mode = minimize(lambda x: -f(x[0]), np.array([x0]), bounds=(bounds,)).x[0]
...
def gen_upper(x: np.ndarray, fx: np.ndarray, dfx: np.ndarray) -> np.ndarray:
...
def eval_upper(x: np.ndarray, fx: np.ndarray, dfx: np.ndarray, y: np.ndarray, x_new: float) -> float:
...
def eval_lower(x: np.ndarray, fx: np.ndarray, x_new: float) -> float:
...
def sample_envelope(x: np.ndarray, fx: np.ndarray, dfx: np.ndarray, y: np.ndarray,
