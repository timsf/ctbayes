...
def eval_grad(thi, fin_t, init_v, fin_v, t, zs):
    ...
    bet, sig = thi
    dbet_phi = 4 * bet * (fin_t * np.square(zs) + np.square(vt / sig) + 2 * np.sqrt(fin_t) * zs * vt / sig) - 1
    dsig_phi = -4 * bet ** 2 * (np.sqrt(fin_t) * zs * vt / sig ** 2 + np.square(vt) / sig ** 3)
...
bounds_thi = (np.array([0, 0]), np.array([np.inf, np.inf]))
